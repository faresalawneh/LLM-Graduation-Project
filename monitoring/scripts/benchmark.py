...
VLLM_MODEL        = "facebook/opt-125m"
VLLM_PORT         = 8000
BURSTGPT_CSV_PATH = Path("/home/hasan/LLM-Graduation-Project/monitoring/BurstGPT_without_fails_3.csv")
PUSHGATEWAY_URL   = "http://localhost:9091"
N_REQUESTS        = 500
REQUEST_RATE      = 10
...
>>>>>>> Add BurstGPT timestamp replay benchmark and update Prometheus config
...
def push_metric(name, value, job="llm_bench"):
    payload = "# TYPE " + name + " gauge\n" + name + " " + str(value) + "\n"
    requests.post(PUSHGATEWAY_URL + "/metrics/job/" + job,
                  data=payload, headers={"Content-Type": "text/plain"}, timeout=5)

async def send_request(session, prompt_tokens, max_tokens, idx):
    payload = {"model": VLLM_MODEL, "prompt": "hello " * prompt_tokens, "max_tokens": max_tokens, "stream": True}
>>>>>>> Add BurstGPT timestamp replay benchmark and update Prometheus config
...
def load_burstgpt(path, n):
    ...
    with open(path) as f:
        ...
        for row in reader:
            ...
            resp_col = None
            for k in row:
                if "request"  in k.lower() and "token" in k.lower():
                    req_col = k
                if "response" in k.lower() and "token" in k.lower():
                    resp_col = k
